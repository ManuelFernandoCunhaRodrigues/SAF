...
function FormField({
  ...
  return (
    <div className="flex flex-col gap-1.5">
      <label htmlFor={id} className="text-sm font-semibold text-zinc-600 dark:text-[#64748B]">
        {label}
      </label>
      <div className="relative">
        <input
          ...
          className="w-full h-11 px-4 rounded-xl border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-[#27272A] text-sm text-zinc-800 dark:text-[#F1F5F9] placeholder:text-zinc-400 dark:placeholder:text-zinc-500 outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 transition-all duration-200"
        />
        {rightSlot && (
          <div className="absolute inset-y-0 right-3 flex items-center">{rightSlot}</div>
        )}
      </div>
      {hint && <p className="text-xs text-zinc-400 dark:text-[#94A3B8]">{hint}</p>}
    </div>
  );
}
    ...
    <FormField
      ...
      rightSlot={
        <button
          type="button"
          onClick={() => setShow((s) => !s)}
          className="text-zinc-400 dark:text-[#94A3B8] hover:text-zinc-600 dark:hover:text-zinc-300 transition-colors"
          tabIndex={-1}
        >
...
function SectionTitle({ Icon, title, subtitle }: { Icon: React.ElementType; title: string; subtitle: string }) {
  return (
    <div className="flex items-start gap-3 mb-6">
      ...
      <div>
        <p className="text-base font-bold text-zinc-800 dark:text-[#F1F5F9]">{title}</p>
        <p className="text-xs text-zinc-500 dark:text-[#64748B] mt-0.5">{subtitle}</p>
      </div>
    </div>
  );
}

const C = "bg-white dark:bg-[#18181B] border border-zinc-100 dark:border-[#27272A] rounded-2xl p-8 shadow-sm";
  ...
  return (
    <div className="py-8 max-w-3xl space-y-6">
      ...
      <div className={C}>
        ...
        <div className="flex items-center gap-5 pb-6 border-b border-zinc-100 dark:border-[#27272A]">
          ...
          <div>
            <p className="text-lg font-semibold text-zinc-800 dark:text-[#F1F5F9] leading-tight">
              {user?.name ?? "Usuário"}
            </p>
            <div className="flex items-center gap-1.5 mt-1">
              <Mail size={12} className="text-zinc-400 dark:text-[#94A3B8]" />
              <p className="text-sm text-zinc-500 dark:text-[#64748B]">{user?.email ?? "—"}</p>
            </div>
            <span className="inline-block mt-2 text-[11px] font-semibold px-2.5 py-0.5 rounded-full bg-blue-50 dark:bg-blue-500/10 text-blue-600 dark:text-blue-400 border border-zinc-100 dark:border-blue-500/20">
              {role}
            </span>
          </div>
        </div>
      ...
      <div className={C}>
        ...
        <p className="text-xs text-zinc-400 dark:text-[#94A3B8] mb-5 flex items-center gap-1.5">
        ...
        <div className="flex items-center gap-3 mt-6">
          <button
            ...
            className="flex items-center gap-2 px-5 py-2.5 bg-blue-900 dark:bg-blue-800 hover:bg-blue-950 dark:hover:bg-blue-700 text-white text-sm font-semibold rounded-xl shadow-sm transition-all duration-200"
          >
      ...
      <div className={C}>
        ...
        <dl className="space-y-0">
          {[
            ...
            <div
              key={item.label}
              className="flex justify-between items-center py-3.5 border-b border-zinc-100 dark:border-[#27272A] last:border-none"
            >
              <span className="text-sm text-zinc-500 dark:text-[#64748B]">{item.label}</span>
